<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold text-ink">Departments</h1>
    </div>
    ...
    <div class="bg-white rounded border border-gray-200 divide-y divide-gray-100">
        @foreach ($departments as $department)
            <div class="p-4 flex items-center justify-between">
                <div>
                    <div class="font-medium text-ink">{{ $department->name }}</div>
                    <div class="text-sm text-steel">{{ $department->code }} · {{ $department->projects_count }} project{{ $department->projects_count === 1 ? '' : 's' }}</div>
                </div>
                <button wire:click="delete({{ $department->id }})" wire:confirm="Delete this department? This cannot be undone." class="text-sm text-signal-rust hover:underline">
                    Delete
                </button>
            </div>
        @endforeach

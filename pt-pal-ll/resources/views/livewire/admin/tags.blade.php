<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-semibold text-ink">Tags</h1>
    </div>
        ...
        @foreach ($tags as $tag)
            <div class="p-4 flex items-center justify-between">
                <div>
                    ...
                    <div class="text-sm text-steel">{{ $tag->lessons_count }} lesson{{ $tag->lessons_count === 1 ? '' : 's' }}</div>
                </div>
                <button wire:click="delete({{ $tag->id }})" wire:confirm="Delete this tag? This cannot be undone." class="text-sm text-signal-rust hover:underline">
                    Delete
                </button>
            </div>
        @endforeach
